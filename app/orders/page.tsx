  ...
  if (orders.length === 0) {
    return (
      <div className="flex flex-col items-center gap-3 text-center py-20 max-w-xs mx-auto">
        ...
        <h2 className="text-lg font-semibold text-slate-700">Belum ada pesanan</h2>
        <p className="text-sm text-slate-400">Anda belum pernah melakukan pemesanan</p>
        <Link href="/program"
      ...
      <div className="space-y-3">
        {orders.map(order => (
          <Link key={order.id} href={`/orders/${order.id}`}
            className="block bg-white rounded-2xl border border-[var(--border-subtle)] shadow-[var(--shadow-xs)] hover:shadow-[var(--shadow-md)] hover:-translate-y-0.5 transition-[box-shadow,transform] duration-200 p-5">
            <div className="flex items-start justify-between mb-3">
              ...
              <span className={`text-xs font-semibold px-3 py-1 rounded-full ${STATUS_COLORS[order.status] || 'bg-slate-50 border border-slate-200 text-slate-700'}`}>
            ...
            <div className="flex items-center justify-between text-sm text-slate-600">
              ...
              <div className="flex items-center gap-2">
                {order.payments?.[0] && (
                  <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${PAYMENT_COLORS[order.payments[0].status] || 'bg-slate-50 border border-slate-200 text-slate-600'}`}>
