...
export default function CartPage() {
  ...
  if (loading) return <div className="text-center py-16 text-gray-400">Memuat keranjang...</div>;
  ...
  if (!cart || cart.items.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-gray-500 mb-6">Tambahkan modul dari program studi Anda</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
          {cart.items.map(item => (
            <div key={item.id} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
              <div className="bg-gray-100 rounded-lg w-16 h-20 flex-shrink-0 flex items-center justify-center overflow-hidden">
                {item.coverImageUrl ? (
              ...
              <div className="flex-1 min-w-0">
                <p className="font-mono text-xs text-blue-600 font-semibold">{item.tboCode}</p>
                <p className="text-sm font-medium text-gray-900 truncate">{item.moduleName}</p>
                <p className="text-sm text-gray-500">{formatIDR(item.priceSnapshot)}</p>
                {!item.isAvailable && (
                  <p className="text-xs text-red-500 mt-1">Modul tidak tersedia lagi</p>
                )}
              </div>
              <div className="flex items-center gap-3 flex-shrink-0">
                <span className="text-sm font-semibold">{formatIDR(item.subtotal)}</span>
                <button
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl border border-gray-200 p-5 sticky top-24">
            ...
            <Link href="/program" className="block w-full text-center text-sm text-gray-500 mt-3 hover:text-gray-700">
              Tambah Modul Lagi
            </Link>
          </div>
        </div>
